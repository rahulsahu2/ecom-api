...

{
    ...
    public function index()
    {
        ...
        $featuredBrandlist = featuredBrands::with('brands')->with('category')->get();
        $cate_arr = [];
        foreach($featuredBrandlist as $popularBrand){

            if(!in_array($popularBrand->category_id, $cate_arr)){
                $cate_arr[] = $popularBrand->category_id;
            }
    ...
    public function blog(Request $request){
        ...
        $blogs = Blog::with('activeComments')->with('Category')->orderBy('id','desc')->where(['status' => 1]);
        if($request->search){
    ...
    public function featured_product(){
        $products = Product::with('activeVariants.activeVariantItems')->orderBy('id','desc')->where(['is_featured'=>1])->get();
        return response()->json(['featuredProducts' => $products]);
    }

    public function new_product(){
        $products = Product::with('activeVariants.activeVariantItems')->orderBy('id','desc')->where(['new_product'=>1])->get();
        return response()->json(['newProducts' => $products]);
    }
    ...
    public function productsReviewList(){

        $reviews = ProductReview::with('product')->where(['status' => 1])->where('rating', '>', 3)->paginate(7);
