...
class BestDiscountProductsController extends Controller
{
    ...
    {
        $shopconcern = BestDiscountProducts::get();
        return view('admin.BestDiscountProducts.index',compact('shopconcern'));
    }

    public function create(){
        $shopconcern = null;
        return view('admin.BestDiscountProducts.edit',compact('shopconcern'));
    }

    public function show($id)
    {
        $shopconcern = BestDiscountProducts::find($id);
        return view('admin.BestDiscountProducts.edit',compact('shopconcern'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->image){
            ...
            $banner_name = 'BestDiscountProducts'.date('-Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;
    ...
    public function store(Request $request){
        ...
        $shopconcern = new BestDiscountProducts();
        if($request->image){
            $extention = $request->image->getClientOriginalExtension();
            $banner_name = 'BestDiscountProducts'.date('-Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;
            $banner_name = 'uploads/custom-images/'.$banner_name;
    ...
    public function changeStatus($id){
        $shop = BestDiscountProducts::find($id);
        if($shop->isactive==1){
            $shop->isactive=0;
            $shop->save();
