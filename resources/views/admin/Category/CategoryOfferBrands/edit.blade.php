...
@section('title')
<title>{{__('category Offers')}}</title>
@endsection
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>{{__('category Offers')}}</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="{{ route('admin.dashboard') }}">{{__('admin.Dashboard')}}</a></div>
              <div class="breadcrumb-item">{{__('category Offers')}}</div>
            </div>
          </div>
​
          <div class="section-body">
          <a href="{{ route('admin.category-offer-brands.index') }}" class="btn btn-primary"><i class="fas fa-list"></i> {{__('category Offers')}}</a>
            <div class="row mt-4">
                <div class="col-12">
                  <div class="card">
                    <div class="card-body">
                        @if ($categoryBanners)
                            <form action="{{ route('admin.category-offer-brands.update',$categoryBanners->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
...
​
                        ...
                        @else
                            <form action="{{ route('admin.category-offer-brands.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="form-group col-6">
                                        <label for="">{{__('admin.New Image')}}</label>
