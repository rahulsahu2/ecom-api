        ...
        <section class="section">
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item">{{__('Section8')}}</div>
            </div>
          </div>

          <div class="section-body">
            <a href="{{ route('admin.create-section8') }}" class="btn btn-primary"><i class="fas fa-plus"></i> {{__('admin.Add New')}}</a>
            <div class="row mt-4">
                <div class="col">
                  <div class="card">
                    <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($shopconcern as $index => $service)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.show-section8',$service->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></a>
...
<script>
    function deleteData(id){
        $("#deleteForm").attr("action",'{{ url("admin/section8/") }}'+"/"+id)
    }
    function changeStatus(id){
        $.ajax({
            type:"put",
            data: { _token : '{{ csrf_token() }}' },
            url:"{{url('/admin/section8-status')}}"+"/"+id,
    ...
    function changeMain(id){
        $.ajax({
            type:"put",
            data: { _token : '{{ csrf_token() }}' },
            url:"{{url('/admin/section8-main')}}"+"/"+id,
            success:function(response){
                toastr.success(response)
            },
            error:function(err){
